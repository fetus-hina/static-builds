...
RUN wget -O /root/nghttp2.tar.xz 'https://github.com/nghttp2/nghttp2/releases/download/v1.40.0/nghttp2-1.40.0.tar.xz' && \
    mkdir /root/nghttp2 && \
    cd /root/nghttp2 && \
...
RUN wget -O /root/c-ares.tar.gz 'https://c-ares.haxx.se/download/c-ares-1.16.1.tar.gz' && \
    mkdir /root/c-ares && \
    cd /root/c-ares && \
...
# https://curl.haxx.se/
RUN wget -O /root/curl.tar.xz 'https://curl.haxx.se/download/curl-7.70.0.tar.xz' && \
    mkdir /root/curl && \
    cd /root/curl && \
